...
3.  Append the following line to <tt>~/.bash_login</tt>:
    ...
    If you are using TextMate, you should also apply {this (hopefully temporary) fix}[http://roman.flucti.com/textmate-fix-for-relative-require-test_helper].
...
Optionally, you can specify options for RailsTestServing to use, by defining <tt>$test_server_options</tt> right before <tt>require 'rails_test_serving'</tt>. It must be a hash with symbol keys. Currently available options are:
